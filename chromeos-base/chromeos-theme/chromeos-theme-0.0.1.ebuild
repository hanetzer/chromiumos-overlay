# Copyright (c) 2009 The Chromium OS Authors. All rights reserved.
# Distributed under the terms of the GNU General Public License v2

EAPI=2

inherit multilib
inherit toolchain-funcs

DESCRIPTION="Chrome OS GTK+ Theme."
HOMEPAGE="http://src.chromium.org"
SRC_URI=""
LICENSE="BSD"
SLOT="0"
KEYWORDS="amd64 x86 arm"
IUSE=""

DEPEND="dev-libs/glib
        >=x11-libs/gtk+-2"

RDEPEND="${DEPEND}"

src_unpack() {
	local theme="${CHROMEOS_ROOT}/src/platform/theme"
	elog "Using theme: $theme"
	mkdir -p "${S}"
	cp -ar "${theme}" "${S}" || die
}

src_compile() {
	if tc-is-cross-compiler ; then
		tc-getCC
		tc-getCXX
		tc-getAR
		tc-getRANLIB
		tc-getLD
		tc-getNM
		export PKG_CONFIG_PATH="${ROOT}/usr/lib/pkgconfig/"
		export CCFLAGS="$CFLAGS"
	fi

  pushd theme
	scons || die "theme compile failed."
	popd
}

src_install() {
  dodir /usr/lib/gtk-2.0/2.10.0/engines
  install -m644 -o root -g root "${S}"/theme/libtheme.so \
    "${D}"/usr/$(get_libdir)/gtk-2.0/2.10.0/engines || die
  
  dodir /usr/share/themes/theme/gtk-2.0
  install -m644 -o root -g root "${S}"/theme/resources/gtkrc \
    "${D}"/usr/share/themes/theme/gtk-2.0 || die
}

pkg_postinst() {
  rm -f "${ROOT}"/etc/gtk-2.0/gtkrc
  ln -s /usr/share/themes/theme/gtk-2.0/gtkrc "${ROOT}"/etc/gtk-2.0/gtkrc
}
