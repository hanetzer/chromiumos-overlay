...
sudo tar jxvpf \
  "${PKGDIR}/${CROSS_TARGET}/cross-${CROSS_TARGET}"/glibc-${GLIBC}.tbz2 \
...
sudo cp -a /usr/lib/gcc/"${CROSS_TARGET}"/*/libgcc_s.so* "${ROOT_FS_DIR}/lib"
sudo cp -a /usr/lib/gcc/"${CROSS_TARGET}"/*/libstdc++.so* "${ROOT_FS_DIR}/usr/lib"
