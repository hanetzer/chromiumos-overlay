...
IUSE="bash-completion debug doc test examples tools"
...
src_prepare() {
	...
	if ! use examples ; then
		epatch "${FILESDIR}"/${P}-remove-examples.patch
	fi
	if use tools ; then
		epatch "${FILESDIR}"/${P}-crosscompile-dbus-binding-tool.patch
	else
		epatch "${FILESDIR}"/${P}-remove-tools.patch
	fi
