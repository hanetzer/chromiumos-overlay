...
src_prepare() {
	...
	cd "${S_K}"
	sed -i -e 's:-Werror::' tools/lib/api/Makefile || die
	#  clang compatible parameters
	# sed -i -e 's:-O6:-O3:' tools/perf/config/Makefile tools/lib/api/Makefile || die
	# find "${S_K}" -name Makefile\* | xargs sed -i -e '/$(CROSS_COMPILE)gcc/d' || die
...
src_install() {
	perf_make -f Makefile.perf install-tools DESTDIR="${D}"
