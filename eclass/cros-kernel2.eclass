...
cros-kernel2_src_install() {
	...
	# Remove the files generated by "depmod". Because the script
	# "build_image" will run "depmod" again after emerging all the
	# packages, kernel packages should not own the outputs of "depmod".
	#
	# "depmod_output_files" is based on "depfiles[]" in
	# http://git.kernel.org/cgit/utils/kernel/module-init-tools/module-init-tools.git/tree/depmod.c .
	if cros_chkconfig_present MODULES; then
		local depmod_output_files=(
			modules.dep
			modules.dep.bin
			modules.pcimap
			modules.usbmap
		...
		local f
		for f in "${depmod_output_files[@]}"; do
			rm -f "${D}/lib/modules/${version}/${f}"
		done
	fi
}
