...
src_prepare() {
	if [ "/usr/x86_64-pc-linux-gnu/gcc-bin/4.6.x-google" != $(gcc-config -B) ]; then
		ewarn "Beware sheriff: the gcc layout changed since last manual check."
		ewarn "This may have led to unusable resulting Clang. Check that it can build a simple hello_world.cc"
	fi
