...
eblit-src_unpack-post() {
	...
	einfo "Applying fix for accept4() on ARM."
	epatch "${FILESDIR}"/2.10/glibc-2.10-accept4.patch

	einfo "Adding 64-byte TLS alignment patch for i386"
	epatch "${FILESDIR}"/2.10/glibc-2.10-tls-alignment.patch

	einfo "Adding memmove patch to fix __restrict bug"
	epatch "${FILESDIR}"/2.10/glibc-2.10-memmove.patch

	einfo "Applying ARM backtrace() support"
	cd "${S}/ports"
	epatch "${FILESDIR}"/2.10/glibc-2.10-backtrace.patch
	...
	if use hardened ; then
		...
		einfo "Applying security fixes for setuid binaries"
		epatch "${FILESDIR}"/2.10/glibc-2.10-no-expand-origin.patch
		epatch "${FILESDIR}"/2.10/glibc-2.10-suid-audit-objects.patch
