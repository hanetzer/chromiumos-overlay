...
eblit-src_unpack-pre(){
	# We do not use all the gentoo patches for glibc.
	# We disable the patching at src_unpack and patch the patches
	# that are needed at eblit-src-unpack-post().
	GLIBC_PATCH_EXCLUDE="*"
}

eblit-src_unpack-post() {
	cd "${S}"
	einfo "Applying Local Glibc Patchset"
	epatch "${WORKDIR}"/patches/0020_all_glibc-tweak-rfc1918-lookup.patch
	epatch "${WORKDIR}"/patches/0030_all_glibc-respect-env-CPPFLAGS.patch
	epatch "${WORKDIR}"/patches/0044_all_glibc-2.10-resolv-nameserver-fallback.patch
	epatch "${WORKDIR}"/patches/0085_all_glibc-disable-ldconfig.patch
	epatch "${WORKDIR}"/patches/1020_all_glibc-longjmp-chk-hidden-fortify.patch
	epatch "${WORKDIR}"/patches/1080_all_glibc-no-inline-gmon.patch
	epatch "${WORKDIR}"/patches/5063_all_glibc-dont-build-timezone.patch
